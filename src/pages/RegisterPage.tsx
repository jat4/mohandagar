  ...
  const handleGoogleSignUp = async () => {
    ...
    try {
      ...
      showToast({
        type: 'success',
        title: 'Account Ready',
        message: 'Welcome to Runner Stopwatch.'
      });
      navigate(redirectUrl, { replace: true });
    } catch (err: any) {
      console.error(err);
    ...
    <div className="min-h-[75vh] flex flex-col items-center justify-center p-4 sm:p-6 animate-fadeIn">
      <div className="w-full max-w-md bg-slate-900 border border-slate-800 rounded-3xl p-6 sm:p-8 shadow-2xl relative">
        ...
        <div className="text-center mb-6">
          <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-emerald-500 to-teal-600 p-[1px] mx-auto mb-3 shadow-lg shadow-emerald-500/20">
          ...
          <p className="text-xs font-mono text-slate-400 mt-1">
            Create an account to host races and sync checkpoints
          </p>
        </div>
        ...
        <form onSubmit={handleEmailSignUp} className="space-y-4">
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full py-3.5 px-4 rounded-xl bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-400 hover:to-teal-500 text-slate-950 font-bold font-mono text-xs sm:text-sm flex items-center justify-center gap-2 shadow-lg shadow-emerald-500/20 transition-all cursor-pointer disabled:opacity-60"
          >
            <UserPlus className="w-4 h-4" />
            <span>{loading ? 'Creating Account...' : 'Create Host Account'}</span>
          </button>
        </form>
